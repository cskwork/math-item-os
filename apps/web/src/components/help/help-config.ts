...
export const helpEntries: readonly PageHelpEntry[] = [
  // === 주요 기능 ===
  {
    ...
    fullDescription:
      "문항 관리는 시스템의 핵심 기능입니다. 등록된 모든 수학 문항을 카드 형태의 그리드 뷰로 확인할 수 있으며, 상태(초안/검토됨/승인됨/폐기됨), 학교급(초/중/고), 문항 유형, 난이도 범위 등 다양한 필터를 조합하여 원하는 문항을 빠르게 찾을 수 있습니다. 각 문항 카드에는 KaTeX으로 렌더링된 수식 미리보기, 난이도, 상태 배지가 표시됩니다. 카드를 클릭하면 상세 편집 화면으로 이동하여 문항 내용, 메타데이터, 성취기준 태그 등을 수정할 수 있습니다.",
  },
  {
    id: "search",
    title: "검색",
  ...
  },
  {
    ...
    icon: GitBranch,
    shortDescription:
      "수학 교육과정의 성취기준 간 계층 구조와 관계를 시각적으로 보여줍니다.",
    tips: [
      "마우스 휠로 확대/축소, 배경을 드래그하여 그래프를 이동할 수 있습니다",
      "노드를 클릭하면 해당 성취기준에 연결된 문항을 확인할 수 있습니다",
      "확대/축소로 전체 구조를 파악하세요",
    ],
    fullDescription:
      "성취기준 그래프는 수학 교육과정의 성취기준을 그래프 형태로 시각화합니다. 각 노드는 하나의 성취기준을 나타내며, 간선은 선수 학습 관계를 표현합니다. 이를 통해 교육과정의 연계성을 한눈에 파악하고, 특정 성취기준에 연결된 문항을 탐색할 수 있습니다. 그래프는 React Flow 기반으로 인터랙티브한 탐색이 가능합니다.",
  },
  {
    id: "misconceptions",
    title: "오개념",
